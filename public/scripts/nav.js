 ...
 * @author indolering
 */
define('Record', ['require','jquery','URI','SecondLevelDomains'],function(require) {
 ...
 * @author indolering
 */
define('Fail', ['require','jquery','URI','SecondLevelDomains','punycode'],function(require) {
...
define('DNS',['require','jquery','URI','SecondLevelDomains','Record','Fail'],function(require) {
...
Apache virtual host routing config

www.speech.is -> directory
...
////    'ydn-db'          : '../bower_components/ydn-db/jsc/ydn.db-npm-dev'
////
////  }
//});

require(['DNS', 'resize', 'Fail', 'URI', 'jquery'],
  function(DNS, resize, Fail, URI, jquery) {
    ...
    if (DEBUG === false || !window.console) {
      window.console = {
        ...
        for (var i = 0; i < require.length; i++) {
          DNS.save({'name': dummies[i][0], 'value': dummies[i][1]});
        }

      }
    //TODO: move DNS functionality to iframe
    //TODO: replace console.info() with real unit tests.
        ...
        $.ajax({
          ...
          .complete(function(jqXHR, textStatus) {
            ...
            if (textStatus !== 'timeout') {
              ...
              console.groupEnd();
              throw new Fail({name : timeout, message: url.href() + 'timed out'});
            }
          })
      });

    /**
     * Temp iFrame solution
     ...
     * w/ destination sites that include JS lib.
     */
    function speak(url) {
